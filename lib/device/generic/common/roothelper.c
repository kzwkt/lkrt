...
static void (*die_fn)(const char *, unsigned int, const char *) =
    die_in_parent;
...
static void *xrealloc(void *const old, const size_t size)
{
    if (size <= 0)
        die();
    if (size >= INT_MAX)
...
static char *xstrndup(const char *const old, const size_t len)
{
    if (old == NULL)
        die();
    if (len >= INT_MAX)
...
static int xopen(const char *const pathname, const int flags)
{
    if (pathname == NULL)
        die();
    if (*pathname != '/')
 ...
 * data we have. */
static size_t gecos_size(const struct gecos_data *const parsed)
{
    if (parsed == NULL)
        die();
...
gecos_parse(const char *const gecos, struct gecos_data *const parsed)
{
    ...
    for (i = 0;; i++) {
        ...
        if (strlen(*dest) != field_len)
            die();

        if (strpbrk(*dest, GECOS_BADCHARS) != NULL && i != 4)
            die();
    ...
    if (gecos_size(parsed) > GECOS_LENGTH)
        die();
}

/* Assemble a new gecos string. */
static const char *gecos_assemble(const struct gecos_data *const parsed)
{
    ...
    xsnprintf(ret, sizeof(ret), "%s,%s,%s,%s,%s",
              parsed->full_name,
              parsed->office,
              parsed->office_phone, parsed->home_phone, parsed->site_info);
...
static char read_request(const int fd, char *const data, const size_t size)
{
    if (fd <= -1)
        die();
    if (data == NULL)
    ...
    if (read(fd, header, sizeof(header) - 1) != sizeof(header) - 1)
        die();
    ...
    if (errno != 0 || endptr != &header[sizeof(header) - 1])
        die();

    if (len >= size)
        die();
    if (read(fd, data, len + 1) != (ssize_t) (len + 1))
        die();
    if (data[len] != '\n')
        die();
    data[len] = '\0';
...
static void sigchild_handler(const int signum __attribute__((__unused__)))
{
    is_child_dead = true;
}
...
static void forkstop_userhelper(struct userhelper *const uh)
{
    if (uh == NULL)
        die();
    if (uh->pid != 0)
        die();
    if (gecos_size(&uh->gecos) > GECOS_LENGTH)
        die();

    struct rlimit fsize;
    if (getrlimit(RLIMIT_FSIZE, &fsize) != 0)
        die();
    if (uh->fsizelim > fsize.rlim_max)
        die();
    if (uh->fsizelim <= 0)
        die();
    ...
    cpu_set_t old_cpus;
    CPU_ZERO(&old_cpus);
    if (sched_getaffinity(0, sizeof(old_cpus), &old_cpus) != 0)
        die();

    {
        ...
        if (sched_setaffinity(0, sizeof(new_cpus), &new_cpus) != 0)
            die();
    }
    ...
    if (uh->pid == 0) {
        ...
        if (close(sv[1]) != 0)
            die();
        if (dup2(sv[0], UH_INFILENO) != UH_INFILENO)
            die();
        if (dup2(sv[0], UH_OUTFILENO) != UH_OUTFILENO)
            die();

        const int devnull_fd = open("/dev/null", O_RDWR);
        if (dup2(devnull_fd, STDIN_FILENO) != STDIN_FILENO)
            die();
        if (dup2(devnull_fd, STDOUT_FILENO) != STDOUT_FILENO)
            die();
        if (dup2(devnull_fd, STDERR_FILENO) != STDERR_FILENO)
            die();
        ...
        if (setpriority(PRIO_PROCESS, 0, +19) != 0)
            die();
        static const struct sched_param sched_param = {.sched_priority =
                0 };
    ...
    for (;;) {
        ...
        if (type == UH_SYNC_POINT)
            break;

        switch (type) {
        case UH_USER:
            if (strcmp(data, my.username) != 0)
                die();
            break;
        case UH_SERVICE_NAME:
            if (strcmp(data, "chfn") != 0)
                die();
            break;
        ...
        case UH_ECHO_OFF_PROMPT:
            ...
            break;
        case UH_EXPECT_RESP:
            if (strtoul(data, NULL, 10) != expected_responses)
                die();
            break;
        default:
            break;
    ...
    if (inotify_add_watch(inotify_fd, BACKUP, IN_CLOSE_WRITE) <= -1) {
        if (errno != ENOENT)
            die();
        if (inotify_add_watch(inotify_fd, ETCDIR, IN_CREATE) <= -1)
            die();
    }
        ...
        for (ep = (struct inotify_event *) (buffer); buflen >= sizeof(*ep);
            ...
            if (ep->len > 0) {
                if (buflen < ep->len)
                    die();
                buflen -= ep->len;
                if ((ep->mask & IN_CREATE) == 0)
                    die();
                (void) inotify_add_watch(inotify_fd, BACKUP,
                                         IN_CLOSE_WRITE);
                continue;
            }
            if (ep->len != 0)
  ...
  stop_userhelper:
    ...
    if (signal(SIGCHLD, SIG_DFL) == SIG_ERR)
        die();
    if (sched_setaffinity(0, sizeof(old_cpus), &old_cpus) != 0)
        die();
}

static void continue_userhelper(struct userhelper *const uh)
{
    if (uh == NULL)
        die();
    if (uh->fd != -1)
...

    {
        ...
        if (WEXITSTATUS(status) !=
            ((uh->fsizelim == RLIM_INFINITY) ? 0 : ERR_UNK_ERROR))
            die();
    }
...
static void create_backup_of_passwd_file(void)
{
...
static void delete_lines_from_passwd_file(void)
{
    struct gecos_data gecos;
...
int main(void)
{
...

    {
        ...
        for (; candidate[0] <= 'z'; candidate[0]++) {
            if (getpwnam(candidate) != NULL)
                continue;
            strcat(candidate_users, candidate);
        }
    }
    if (candidate_users[0] == '\0')
        die();
    ...
    if (fputs("Please enter your password:\n", stdout) == EOF)
        die();
    if (fgets(my.password, sizeof(my.password), stdin) == NULL)
        die();
...
generate_userhelper(const char *const a, const int i, char *const contents)
{
    ...
    const rlim_t fsizelim =
        (a[i + 1] == '\0') ? RLIM_INFINITY : target + 1;
    if (fsizelim < passwd_fsize)
        die();

    const size_t contents_len = strlen(contents);
    if (contents_len < passwd_fsize)
        die();
    if (contents_len <= fullname_off)
        die();

    char *const fullname = contents + fullname_off;
    if (memcmp(fullname - before_fullname_len,
               before_fullname, before_fullname_len) != 0)
        die();
    ...
    if (memcmp(rest - after_fullname_len,
               after_fullname, after_fullname_len) != 0)
        die();
    ...
    for (offset = fullname_off; offset < contents_len; offset++) {
        ...
        if (x == '\0')
            die();
        if (is_user_wildcard) {
            if (strchr(candidate_users, x) == NULL)
                continue;
            superuser_elect = x;
        } else {
            if (x != c)
        ...
        if (new_fullname_len < 0)
            continue;           /* gecos_size() > GECOS_LENGTH */
        if (4 + new_fullname_len + siteinfo_len + 1 > GECOS_LENGTH)
            continue;

        if (offset < fullname_off + fullname_len) {
            if (offset != fullname_off + fullname_len - 1)
                die();
            if (new_fullname_len == 0)
                continue;
        }
        ...
        {
            char *const new_contents =
                xmalloc(contents_len + 1 + GECOS_LENGTH);

            memcpy(new_contents, contents, fullname_off);
            generate_fullname(new_contents + fullname_off,
                              new_fullname_len, c);
            ...
            if (strlen(new_contents) != contents_len +
                (new_fullname_len - fullname_len))
                die();

            if (fsizelim != RLIM_INFINITY) {
                if (fsizelim >= strlen(new_contents))
                    die();
                if (fsizelim >= contents_len)
                    die();
                memcpy(new_contents + fsizelim,
                       contents + fsizelim, contents_len + 1 - fsizelim);
            }
        ...
        if (new_fullname_len >= GECOS_LENGTH)
            die();
...
static int generate_userhelpers(const char *const _a)
{
    char a[GECOS_LENGTH];
    if (_a == NULL)
        die();
    ...
    if (read(fd, contents, passwd_fsize) != (ssize_t) passwd_fsize)
        die();
    xclose(fd);
    contents[passwd_fsize] = '\0';
    if (strlen(contents) != passwd_fsize)
        die();
    ...
    for (i = 0; i <= 5; i++) {
        ...
        switch (i) {
        case 0:
            if (field_len != strlen(my.username))
                die();
            if (memcmp(field, my.username, field_len) != 0)
                die();
            break;
        case 1:
            if (*field != 'x')
            ...
            break;
        case 2:
            if (strtoimax(field, NULL, 10) != getuid())
                die();
            break;
        case 3:
            if (strtoimax(field, NULL, 10) != getgid())
                die();
            break;
        case 4:
            {
                ...
                xsnprintf(gecos.site_info, sizeof(gecos.site_info),
                          "%s", my.gecos.site_info);
                if (strcmp(assembled, gecos_assemble(&gecos)) != 0)
                    die();
            }
