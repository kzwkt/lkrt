...
ifneq ($(words $(subst :, ,$(CURDIR))), 1)
  $(error src directory cannot contain spaces nor colons)
endif
...
# Basic helpers built in util/
PHONY += util_basic
util_basic:
	$(Q)$(MAKE) $(build)=util/basic

# To avoid any implicit rule to kick in, define an empty command.
util/basic/%: util_basic ;
...
ifneq ($(KBUILD_SRC),)
	...
	$(Q)$(CONFIG_SHELL) $(srctree)/util/mkmakefile \
	    $(srctree) $(objtree) $(VERSION) $(PATCHLEVEL)
endif

# To make sure we do not include .config for any of the *config targets
# catch them early, and hand them over to util/kconfig/Makefile
...
no-dot-config-targets := clean mrproper distclean \
			 ...
			 $(version_h) headers_% archheaders archutil \
...
# in util/kconfig to make the *config target
...
config: util_basic outputmakefile FORCE
	$(Q)$(MAKE) $(build)=util/kconfig $@
...
# Carefully list dependencies so we do not try to build util twice
# in parallel
PHONY += util
util: util_basic include/config/auto.conf include/config/tristate.conf
	$(Q)$(MAKE) $(build)=$(@)
...
# Use make W=1 to enable them (see util/Makefile.build)
...
# check for 'asm goto'
ifeq ($(shell $(CONFIG_SHELL) $(srctree)/util/gcc-goto.sh $(CC)), y)
	KBUILD_CFLAGS += -DCC_HAVE_ASM_GOTO
	KBUILD_AFLAGS += -DCC_HAVE_ASM_GOTO
endif

include util/Makefile.extrawarn
...
# version.h and util_basic is processed / created.
...
headerdep:
	...
	$(srctree)/util/headerdep.pl -I$(srctree)/include
...
mrproper-dirs      := $(addprefix _mrproper_, util)
...
# Shorthand for $(Q)$(MAKE) -f util/Makefile.clean obj=dir
# Usage:
# $(Q)$(MAKE) $(clean)=dir
clean := -f $(if $(KBUILD_SRC),$(srctree)/)util/Makefile.clean obj
...
help:
	...
	@$(MAKE) -f $(srctree)/util/kconfig/Makefile help
	@echo  ''
...
includecheck:
	find $(srctree)/* $(RCS_FIND_IGNORE) \
		...
		| xargs $(PERL) -w $(srctree)/util/checkincludes.pl
...
%.s: %.c prepare util FORCE
	$(Q)$(MAKE) $(build)=$(build-dir) $(target-dir)$(notdir $@)
%.i: %.c prepare util FORCE
	$(Q)$(MAKE) $(build)=$(build-dir) $(target-dir)$(notdir $@)
%.o: %.c prepare util FORCE
	$(Q)$(MAKE) $(build)=$(build-dir) $(target-dir)$(notdir $@)
%.lst: %.c prepare util FORCE
	$(Q)$(MAKE) $(build)=$(build-dir) $(target-dir)$(notdir $@)
%.s: %.S prepare util FORCE
	$(Q)$(MAKE) $(build)=$(build-dir) $(target-dir)$(notdir $@)
%.o: %.S prepare util FORCE
	$(Q)$(MAKE) $(build)=$(build-dir) $(target-dir)$(notdir $@)
%.symtypes: %.c prepare util FORCE
	$(Q)$(MAKE) $(build)=$(build-dir) $(target-dir)$(notdir $@)
